...
5. Re-reads the task instance and recomputes `is_completed`: `true` once every `photo_requirements` entry has `is_uploaded: true`. `completed_at` is set the moment it flips to `true`.

**Tasks with `is_photo_required: false` are NOT auto-completed on shift creation.** A task snapshot always starts as `is_completed: false`, `completed_at: null`, regardless of whether it needs a photo — photo-required tasks complete via the flow above, and no-photo tasks require the assigned worker to explicitly call `PATCH /shift/:planId/:date/tasks/:taskId/complete` (`markShiftTaskComplete`). That endpoint verifies the worker is assigned to the shift, rejects tasks that do require a photo (400 — those must go through the photo endpoint instead), rejects a task that's already completed, then sets `is_completed: true`/`completed_at` directly and re-runs the same shift-auto-completion check as the photo path. There is still no manager approval step for either path — that remains a deliberate simplification (see below) that could be layered on later (e.g. an `is_approved` flag) without disturbing how completion itself is recorded.

**Once every task on a shift has `is_completed: true`, the shift's own `status` auto-advances to `'completed'`** — the same atomic-and-gated pattern as the check-in → `in_progress` transition in §13: a helper re-reads the shift's `tasks[]` after each photo upload, and if all of them are `is_completed`, runs `updateOne({ _id, status: { $in: ['upcoming', 'in_progress'] } }, { $set: { status: 'completed' } })`. The `$in` guard means this can never re-fire once already completed and can never revive a `cancelled` shift by completing its tasks. A shift with an empty `tasks[]` (no active tasks on its rooms) never auto-completes this way — there being nothing to judge completion by is treated as "not applicable," not "vacuously done."
...
For the common flow — a worker taps a shift from this list to see its detail — no second request is needed: each list item already carries the full shape (rooms, tasks with photo_requirements, assigned_workers, everything), since the list and the single-occurrence read are built by the same `buildShiftSnapshot`/materialization logic. `GET /shift/:planId/:date` (below) exists for the other case: a worker landing directly on a shift's detail screen without having gone through the list first (a push notification deep-link, a page refresh) — it accepts a worker token in addition to a manager's, with an ownership check (403 unless that worker is actually in `assigned_workers`) so a worker can't fetch a shift they aren't part of just by knowing its `planId`/`date`.
...
**Check-in state lives directly on the `assigned_workers` entry** (`check_in_at`, `check_in_coordinates`, `check_out_at`, `check_out_coordinates`) rather than as a separate record, because it's inherently per-worker-per-shift data — the same place that already snapshots who was assigned to work that day. Coordinates are stored (not just the timestamp) as evidence of *where* the check-in actually happened, in case it's ever disputed.

A worker can only check in once and check out once per shift — enforced **atomically**, not just by an earlier read: the update's own query filter uses `$elemMatch: { worker, check_in_at: null }` (check-out: `check_in_at: { $ne: null }, check_out_at: null`), tying both conditions to the same array entry in one query. If two check-in requests from the same worker land at nearly the same instant, only one `findOneAndUpdate` can match and write — the second finds no matching document (because the first request already cleared `check_in_at: null`) and gets rejected with "Already checked in," rather than silently overwriting the first request's timestamp. `$elemMatch` specifically (not two plain dot-path conditions) matters here — without it, Mongo would allow `worker` to match one array element and `check_in_at: null` to match a *different* one, which isn't what's being checked.

**The shift's own `status` auto-advances on the first check-in.** After a successful check-in write, a second, separate atomic update runs: `updateOne({ _id, status: 'upcoming' }, { $set: { status: 'in_progress' } })`. Two things about this are deliberate:
- It's **conditioned on the shift's current status being `'upcoming'`**, not on "a check-in just happened" — so the 2nd, 3rd, etc. worker checking in later (once the shift is already `in_progress`) doesn't re-trigger anything, and a check-in can never reopen a shift that's already `completed`/`cancelled`.
- It's a **separate atomic operation from the check-in write itself**, gated by its own `status: 'upcoming'` filter — so if two workers' first check-ins race each other, only one of these status-update queries actually matches and writes; the other matches zero documents (because the first one already flipped it) and is a harmless no-op, not a duplicate transition or an error.

This is the only automatic status transition in the system — every other `status` change still goes through the manual `PATCH .../status` endpoint (see §7).

**Check-out, by contrast, is gated on `status` rather than advancing it.** A worker cannot check out until the shift's own `status` is already `'completed'` — which only happens once every task's `is_completed` is true (§9's auto-completion). The intent: workers stay checked in for the full duration of the actual cleaning work, and only check out once the shift as a whole is finished, not merely whenever they personally feel done. This precondition is enforced twice, consistently with the check-in/check-out pattern elsewhere in this section: an early read-based check for a clear error message, and again inside the atomic `findOneAndUpdate`'s own query filter (`status: 'completed'`) so a shift that somehow stops being `'completed'` between the read and the write (e.g. a manager manually reverting it via `PATCH .../status`) can't be checked out of on a stale read.

## 14. Keeping today's already-materialized shift in sync with plan/task edits

§4 explains why a materialized `Shift` snapshots rather than live-references its plan — but that snapshot has one practical consequence worth calling out: a future occurrence is still virtual (built fresh from the plan's *current* state every time it's viewed, §2) so an edit to the plan's roster or its rooms' tasks shows up there automatically. **Today's shift, once materialized, does not** — it's already a frozen document, so a plan/task edit made after materialization would otherwise be invisible to it until the day rolls over. Two narrow, targeted resyncs close that gap for today specifically, without touching anything else about the snapshot model:

- **`resyncTodayShiftWorkersIfDue`** — called after a plan's `assigned_workers` is updated. If today's shift is already materialized (and not `is_worker_overridden` — a manager's per-day override via `PATCH .../assign-workers` is more specific than the plan's default roster and must win), its `assigned_workers` is rewritten to match the plan's new roster, preserving each remaining worker's `check_in_at`/`check_out_at` so this can never erase in-progress attendance.
- **`resyncTodayShiftTasksForRoomsIfDue`** — called after a `Task` is created, updated, or (soft-)deleted, with the affected room(s). For every active, non-completed plan whose `rooms` includes one of those rooms, if today's shift is materialized **and still `'upcoming'`**, its `tasks[]` is rewritten to match that room's current active Tasks: a newly created task is added as a fresh (`is_completed: false`) entry, a deleted task's entry is dropped, and a task that still exists has its `name`/`duration_minutes`/`is_photo_required`/`photo_requirements` refreshed from the Task's current fields. `photo_requirements` is re-merged **by `title`**, not replaced outright: a requirement the worker already uploaded keeps its `photo_url`/`is_uploaded` (an edit never erases upload progress already made today), a brand-new title starts unset, and a removed title simply drops out. `is_completed` is then recomputed for a photo-required task the normal way (true once every merged requirement is uploaded) — but for a no-photo task it's left exactly as it was, since that one only ever changes via the worker's explicit mark-complete action (§9), never as a side effect of a plan/task edit. `duration_minutes` on the shift itself is recomputed to match.

Both resyncs are **deliberately no-ops once work is underway or done**: the workers resync only touches a shift that isn't overridden, and the tasks resync only fires for `status: 'upcoming'` — an `in_progress`, `completed`, or `cancelled` shift is left exactly as-is. A plan/task edit should never retroactively change what a team is mid-way through cleaning, or rewrite the record of what already happened. If the edit needs to take effect for that room/plan today after work has started, it simply won't — it applies from the next occurrence onward, same as every other plan edit already does once materialization has happened (e.g. a plan's own `rooms` list changing, which these resyncs don't cover).
